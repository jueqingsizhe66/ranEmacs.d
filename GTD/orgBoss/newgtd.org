...
** TODO [#B] Learning English with Mrs xiao xinran <2017-12-05 周二 14:11 +1m> :@home:
...
*** TODO [#A] 修改静态偏航工况的文章，再投稿 <2018-06-23 周六 11:47>  [/] :学术:
...
**** TODO [#A] CFX静态偏航补充计算 <2018-06-25 周一 08:30>      [0%]   :学术:
DEADLINE: <2018-07-01 周日>
                    :PROPERTIES:
                    :Effort: 4:00
                    :END:
                    :LOGBOOK:
...
1. [-] 不同网格的验证，选择一种湍流模型
   ...
   2. [X] SST
   3. [ ] T-SST
2. [-] 选择完网格之后，再进行湍流模型的验证
   1. [ ] 250w
   2. [X] 370w
   3. [ ] 490w
3. [ ] 验证完湍流模型之后，为了表示非定常时间步选择的合理性，再考虑使用不同时间步
4. [-] 负偏航角计算(-30°)
...
**** TODO [#A] Fluent双滑移的验证case <2018-06-25 周一 08:36>     [0%] :学术:
DEADLINE: <2018-07-14 周六>
                    :PROPERTIES:
                    :Effort: 14:00
                    :END:
                    :LOGBOOK:
...
Numeca激活完毕
  1. [-] 不同网格的验证，选择一种湍流模型(无论Fluent还是CFX定常计算的结果流场不对)
     1. [X] k-omega SST(2)
     2. [X] k-kl SST(3)
...
**** TODO [#A] 双滑移的数据处理 <2018-06-25 周一 08:38>        [0%]    :学术:
DEADLINE: <2018-07-03 周二>
                    :PROPERTIES:
                    :Effort: 4:00
                    :END:
                    :LOGBOOK:
...
4. [ ] 动态截面气动性能处理
   1. [ ] 平均Cp
   2. [ ] Cn Ct 分布
5. [ ] 静态和动态截面云图对比(如果分析出来 气动性能有差别，可以从定性角度先进性观察)


*** TODO [#A] NREL 5MW fluent静态和动态偏航计算 <2018-06-25 周一 08:42>[/] :学术:
...
7. [ ] 动态数据结果的处理
8. [ ] 静态-动态的数据对比处理
9. [ ] NREL5MW相关的几何数据、模拟数据、计算结果等
   1. [ ] 几何数据
   2. [ ] 模拟数据
   3. [ ] 计算结果
****  其他计算 [%]
10. [ ] Free vortex method and BEM (不同偏航角下)
    1. [ ] Free vortex method
    2. [ ] BEM
**** NREL 5MW偏航+变桨实验 [/]
***** 偏航实验 [%]
